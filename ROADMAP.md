...
- Routed ROI captures directly into `<experimentDir>/blobs/images/` with the human-readable `roi-<safeName>-<refShortHash>-<captureId>.png` naming -- exactly one on-disk PNG per ROI capture. The image-artifacts blob pass records sha256 metadata for integrity on the event but no longer renames or copies the file. No-active-experiment captures fall back to `~/.quailbot-pi/observations-orphan/`. The old `~/.quailbot-pi/roi-observations/` directory is no longer created or written to.
